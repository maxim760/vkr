...
import { IBasketItem, selectBasketActions } from 'src/store/order/basketStore'
import MinusIcon from '@mui/icons-material/Remove';
import PlusIcon from '@mui/icons-material/Add';
import { Box, Typography } from '@mui/material';
type IProps = {
  basketItem: IBasketItem | null,
  item: IGoods,
...
export const BasketActions: FC<IProps> = ({ basketItem, item, actions }) => {
  ...
  return (
    <Box sx={{display: "flex", alignItems: "center"}}>
      <AppButton sx={{borderRadius: 2, minWidth: 0, px: 0.5, py: 0.25}} noMargin variant="outlined" size="small" onClick={onClickMinus}>
        <MinusIcon />
      </AppButton>
      <Typography fontSize="0.875rem" sx={{mx: 0.75}}>{basketItem.count} шт.</Typography>
      <AppButton sx={{borderRadius: 2, minWidth: 0, px: 0.5, py: 0.25}} noMargin variant="outlined" size="small" onClick={onClickPlus}>
        <PlusIcon />
      </AppButton>
    </Box>
  )
