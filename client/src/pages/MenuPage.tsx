...
import { selectBasket, selectBasketActions, useBasketStore } from 'src/store/order/basketStore'
...
export const MenuPage: React.FC<IProps> = ({ }) => {
  ...
  const {methods, queryData, invalidateQuery} = useMenuSearch({onlyActive: true})
    ...
    <Layout title="Меню">
      ...
      {!!hasOnlyData(data, { isLoading, error }) && (
        <>
          ...
          <Grid container spacing={2} sx={{mt: 2, alignItems: "stretch", mb: 8}}>
            {isRefetching ? <Loader /> : (
              <>
                {data.map((item) => (
                  <Grid item xs={12} sm={6} md={4} lg={3} key={item.id}> 
                    <GoodsCard item={item} showCount={false}>
                      <BasketActions actions={actions} basketItem={basket[item.id] || null} item={item} />
                    </GoodsCard>
                  </Grid>
                ))}
                {!data.length && (
