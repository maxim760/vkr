...
import { goto } from '$app/navigation';
  ...
  const mutationFolder = createMutation({ 
    mutationFn: () => spaceApi.addUser(name),
    onSuccess: () => {
      toastEvents.success();
...
<h3 class="text-xl font-medium text-gray-900 dark:text-white">Добавление в пространство</h3>
<Label class="space-y-2 flex flex-col">
  <span>email пользователя</span>
  <Input bind:value={name} type="text" name="name" required />
</Label>

<Button type="submit" class="w-full">Добавить</Button>
</form>
