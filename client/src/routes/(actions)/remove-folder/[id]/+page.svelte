...
import { goto } from '$app/navigation';
  ...
  const mutationFolder = createMutation({ 
    mutationFn: () => folderApi.delete($page.params?.id ?? ''),
    onSuccess: () => {
...
<h3 class="text-xl font-medium text-gray-900 dark:text-white">Удаление папки - {folder.name}</h3>
<Button type="submit" class="w-full">Подтвердить</Button>
</form>
