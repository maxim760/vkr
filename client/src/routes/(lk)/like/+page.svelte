<script lang="ts">
  ...
  $: items = $likeRecipes.filter(likeR =>  recipes.find(f => f.recipes.find(r => r.id === likeR?.id)));
</script>

<div class="container">
  <div class="actions">
    <ButtonGroup>
      <Button href="/" outline color="blue">Назад к рецептам</Button>
    </ButtonGroup>
  </div>
  ...
  <div class="items">
      <div class="flex items-center gap-8">
        <div class="flex-none">
          <Heading tag="h5">Любимые рецепты</Heading>
        </div>
      </div>
      <div class="group">
